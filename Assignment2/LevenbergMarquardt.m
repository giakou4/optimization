function [x, y, F] = LevenbergMarquardt(f, e, x0, y0, ALGORITHM, g, KMAX)
% function [x, y, F] = LevenbergMarquardt(f, e, x0, y0, ALGORITHM, g, KMAX)
% LEVENBERGMARQUARDT is a method that tries to minimize a function using 
% the inverted hessian matrix plus something multiplied by the grad of 
% that function (improvement of Newton's Method)
% Inputs
...
while (norm(subs(gradf,{xx,yy}, [x(k),y(k)])) > e && k<KMAX)
    ...
    eigenvalues = eig(H);
    m = abs(max(eigenvalues));
    m = m + 0.05; % m > abs(max(eigenvalues))
    hes = H + m*eye(2);
    while det(hes)<0 || hes(1,1)<0
        m = m + 0.05; % find m such as H+m*I>0
        hes = H + m*eye(2);
    end
    d = -inv(H + m*eye(2)) * double(subs(gradf, {xx, yy}, [x(k),y(k)]))';       
    switch ALGORITHM
        case 1
            g = g;
        case 2
            h = @(g) double(subs(f, {xx, yy}, [x(k)+g*d(1),y(k)+g*d(2)]));
            [g1,g2] = Golden(h,0,2,0.0001,inf); % new g=argmin(f(x-g*gradf(x))) 
            g = (g1(end)+g2(end))/2;
        case 3
            g = Armijo(f,d',x,y,k);
    end
