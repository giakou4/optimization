% Minimize f(x,y) = (x^3)*exp(-x^2-y^4) using Levenberg-Marqardt's method 
...
dir = [pwd '\PlotsLevenbergMarqardt\']; 
...
for i=1:length(x0)
    ...
    iter = [];
    for g=0.1:0.2:2
            [x, y, F] = LevenbergMarquardt(f, 0.001, x0(i), y0(i), 1, g, 1000);
    ...
    [x, y, F] = LevenbergMarquardt(f, e, x0(i), y0(i), 1, g, inf);
    disp(['Number of Iteration for Algorithm #1: ',num2str(length(x))]);
    PlotStem3(f,x,y,F); title(['Levenberg-Marqardt Algorithm #1 Point #',num2str(i)])
    saveas(gcf,[dir 'Algorithm_1_Point_',num2str(i),'_Stem.png'])
    PlotContour(f,x,y,F); title(['Levenberg-Marqardt Algorithm #1 Point #',num2str(i)])
    saveas(gcf,[dir 'Algorithm_1_Point_',num2str(i),'_Contour.png'])

    [x, y, F] = LevenbergMarquardt(f, e, x0(i), y0(i), 2, NaN, inf);
    disp(['Number of Iteration for Algorithm #2: ',num2str(length(x))]);
    PlotStem3(f,x,y,F); title(['Levenberg-Marqardt Algorithm #2 Point #',num2str(i)])
    saveas(gcf,[dir 'Algorithm_2_Point_',num2str(i),'_Stem.png'])
    PlotContour(f,x,y,F); title(['Levenberg-Marqardt Algorithm #2 Point #',num2str(i)])
    saveas(gcf,[dir 'Algorithm_2_Point_',num2str(i),'_Contour.png'])

    [x, y, F] = LevenbergMarquardt(f, e, x0(i), y0(i), 3, NaN, inf);
    disp(['Number of Iteration for Algorithm #3: ',num2str(length(x))]);
    PlotStem3(f,x,y,F); title(['Levenberg-Marqardt Algorithm #3 Point #',num2str(i)])
    saveas(gcf,[dir 'Algorithm_3_Point_',num2str(i),'_Stem.png'])
    PlotContour(f,x,y,F); title(['Levenberg-Marqardt Algorithm #3 Point #',num2str(i)])
