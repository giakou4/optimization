...
for i = 1:length(l_arr)
    ...
    [~,~,calls] = myGolden(f1,a0,b0,l,inf);
    f1_calls = [f1_calls calls];
    
    [~,~,calls] = myGolden(f2,a0,b0,l,inf);
    f2_calls = [f2_calls calls];
    
    [~,~,calls] = myGolden(f3,a0,b0,l,inf);
...
suptitle('Golden Section Method: Calls')
...
for i = 1:length(l_arr)
    l = l_arr(i);
    figure(2)
    suptitle('Golden Section Method for f_1(x) ')
    subplot(length(l_arr),1,i)
    [a,b,~] = myGolden(f1,a0,b0,l,inf);
    plot_intervals(a,b,3,['Tolerance=',num2str(l)])
    
    figure(3)
    suptitle('Golden Section Method for f_2(x) ')
    subplot(length(l_arr),1,i)
    [a,b,~] = myGolden(f2,a0,b0,l,inf);
    plot_intervals(a,b,4,['Tolerance=',num2str(l)])
   
    figure(4)
    suptitle('Golden Section Method for f_3(x) ')
    subplot(length(l_arr),1,i)
    [a,b,~] = myGolden(f3,a0,b0,l,inf);;
