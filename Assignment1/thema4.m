%% Thema 1
% Bisection with derivative method
...
for i = 1:length(l_arr)
    ...
    [~,~,calls] = myBisectionDerivative(f1,a0,b0,l);
    f1_calls = [f1_calls calls];
    
    [~,~,calls] = myBisectionDerivative(f2,a0,b0,l);
    f2_calls = [f2_calls calls];
    
    [~,~,calls] = myBisectionDerivative(f3,a0,b0,l);
...
suptitle('Bisection with Derivative Method: Calls')
...
for i = 1:length(l_arr)
    l = l_arr(i);
    figure(2)
    suptitle('Bisection with Derivative Method for f_1(x) ')
    subplot(length(l_arr),1,i)
    [a,b,~] = myBisectionDerivative(f1,a0,b0,l);
    plot_intervals(a,b,3,['Tolerance=',num2str(l)])
    
    figure(3)
    suptitle('Bisection with Derivative Method for f_2(x) ')
    subplot(length(l_arr),1,i)
    [a,b,~] = myBisectionDerivative(f2,a0,b0,l);
    plot_intervals(a,b,4,['Tolerance=',num2str(l)])
   
    figure(4)
    suptitle('Bisection with Derivative Method for f_3(x) ')
    subplot(length(l_arr),1,i)
    [a,b,~] = myBisectionDerivative(f3,a0,b0,l);
